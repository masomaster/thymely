...
- **Supabase (local, no account needed):** the app runs without a backend (shows a "Connect Supabase" banner) but any read/write needs Supabase. For local data, run the Supabase CLI stack: `supabase start`. This needs the Docker daemon running (start it with `sudo dockerd &` if `docker ps` fails) and the CLI on PATH (it is installed at `~/.local/share/supabase/` — a shim that must stay co-located with its `supabase-go` binary). `supabase start` reads `supabase/config.toml` and applies everything in `supabase/migrations/`. Point the app at it via a `.env` (gitignored) with `EXPO_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321` and the anon key printed by `supabase start` (or `supabase status`). Studio UI is at `http://127.0.0.1:54323`.
